...
export default async function AgentsPage() {
  ...
  for (const payment of payments) {
    ...
    current.push({
      agentPayoutCents: payment.agentPayoutCents,
    });
  ...
  const roster = agents.map((agent) => {
    ...
    try {
      const parsed = JSON.parse(agent.skillsJson);
      if (Array.isArray(parsed)) {
        skills = parsed.filter((item) => typeof item === "string");
      }
    } catch {
      skills = [];
    }
    ...
    const online =
      !!agent.lastSeenAt && now - agent.lastSeenAt.getTime() < 30_000;

    const agentPayments = paymentsByAgent.get(agent.id) ?? [];
    const agentTasks = tasksByAgent.get(agent.id) ?? [];
    const reputation = calculateAgentReputation(agentTasks, agentPayments);
  ...
  return <AgentRoster agents={roster} signedIn={Boolean(userId)} />;
}
