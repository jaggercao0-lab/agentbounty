...
function presetCheck(
  type: "BUILD" | "TESTS" | "LINT",
  criterion: string,
  githubChecks: GitHubCheckRun[] | null,
  githubChecksError: string | null
): VerificationCheck {
  if (githubChecksError) {
    return {
      type,
      criterion,
      status: "FAIL",
      detail: githubChecksError,
    };
  }
  ...
  if (!githubChecks) {
    return {
      ...
      criterion,
      status: "FAIL",
      detail: "GitHub checks were unavailable",
    };
  }

  const matching = githubChecks.filter(
    check => matchesPreset(type, check.name || "")
  );
  ...
  if (pending) {
    return {
      ...
      criterion,
      status: "PENDING",
      detail: `Waiting for: ${names}`,
    };
  }

  const passed = matching.every(
...
export async function runVerification(
  ...
  const prValid =
    ...
    !pullRequest.draft &&
    pullRequest.base.repo.full_name === githubRepo;

  checks.push({
    type: "PULL_REQUEST",
    criterion: "Pull request is valid",
    status: prValid ? "PASS" : "FAIL",
    detail: prValid
      ? pullRequestUrl
  ...
  let githubChecks: GitHubCheckRun[] | null = null;
  let githubChecksError: string | null = null;
  ...
  if (!checksResponse.ok) {
    ...
    githubChecks = Array.isArray(data.check_runs)
      ? data.check_runs
      : [];
  }

  for (const rawCriterion of criteria) {
    const criterion = parseCriterion(rawCriterion);

    if (criterion.type === "PULL_REQUEST") {
      checks.push({
        type: "PULL_REQUEST",
    ...
    if (criterion.type === "CONTENT_CONTAINS") {
      const content = await readRepositoryFile(
        owner,
        repo,
        criterion.path,
      ...
      const found =
        content !== null &&
        content.includes(criterion.value);
    ...
    if (criterion.type === "CONTENT_PRESERVED") {
      ...
      if (baseContent !== null && headContent !== null) {
        const originalLines = baseContent
          .split("\n")
          .map(line => line.trim())
          .filter(Boolean);

        preserved = originalLines.every(
          line => headContent.includes(line)
        );
      }
    ...
    if (allowManualCriteria) {
      manualCriteria.push(criterion.raw);
      continue;
    }
  ...
  if (githubChecksError) {
  ...
  } else if (!githubChecks || githubChecks.length === 0) {
    checks.push({
      type: "GITHUB_CHECKS",
      criterion: "GitHub checks",
      status: "PASS",
      detail: "No GitHub checks are configured for this commit",
    });
  } else {
    const pending = githubChecks.some(
      check => check.status !== "completed"
    );
    ...
    if (pending) {
      ...
      const passed = githubChecks.every(
        check =>
